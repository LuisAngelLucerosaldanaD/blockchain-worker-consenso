...
func (w Worker) doWork() {
	...
	if lotteryActive != nil {
		...
		if int(lifeLottery) > (e.App.SubscriptionTime * 1000) {
			endDate := time.Now()
			lotteryUpdated, _, err := w.Srv.SrvLottery.UpdateLotteryTable(lotteryActive.ID, lotteryActive.BlockId, lotteryActive.RegistrationStartDate, &endDate, &endDate, nil, nil, 26)
			if err != nil {
				logger.Error.Printf("error iniciando la loteria: %s", err)
				return
			}

			participants, _, err := w.Srv.SrvParticipants.GetParticipantsTableByLotteryID(lotteryActive.ID)
			if err != nil {
			...
			for _, participant := range participants {
				if contains(miners, participant.ID) {
					_, _, err = w.Srv.SrvParticipants.UpdateParticipantsTable(participant.ID, lotteryActive.ID, participant.WalletId, participant.Amount, true, 23, false)
					if err != nil {
						logger.Error.Printf("error actulizando el registro del participante: %s", err)
						return
					}
				} else if contains(validators, participant.ID) {
					_, _, err = w.Srv.SrvParticipants.UpdateParticipantsTable(participant.ID, lotteryActive.ID, participant.WalletId, participant.Amount, true, 24, false)
					if err != nil {
						logger.Error.Printf("error actulizando el registro del participante: %s", err)
						return
					}
			...
			endDate = time.Now()
			_, _, err = w.Srv.SrvLottery.UpdateLotteryTable(lotteryUpdated.ID, lotteryUpdated.BlockId, lotteryUpdated.RegistrationStartDate, lotteryUpdated.RegistrationEndDate, lotteryUpdated.LotteryStartDate, &endDate, nil, 27)
			if err != nil {
				logger.Error.Printf("error iniciando la loteria: %s", err)
				return
			}
	...
	connBk, err := grpc.Dial(e.AuthService.Port, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
	...
	_, _, err = w.Srv.SrvLottery.CreateLotteryTable(uuid.New().String(), block.Id, &startDate, nil, nil, nil, nil, 25)
	if err != nil {
		logger.Error.Printf("No se pudo obtener el bloque a minar, error: %s", err)
	}
