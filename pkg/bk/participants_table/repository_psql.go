...
// GetByID consulta un registro por su ID
func (s *psql) getByLotteryID(lotteryId string) ([]*ParticipantsTable, error) {
	const psqlGetByLotteryID = `SELECT id , lottery_id, wallet_id, amount, accepted, type_charge, returned, created_at, updated_at FROM bk.participants_table WHERE lottery_id = '%s' limit 1`
	var ms []*ParticipantsTable
	err := s.DB.Select(&ms, fmt.Sprintf(psqlGetByLotteryID, lotteryId))
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
